...
`POST /api/proxy` forwards any HTTP request it is handed, and it has **no
authentication of its own**. Reachable from the internet without a gate in
front, it is an open HTTP proxy: it will be found by scanners, used to relay
traffic through your IP, and get the instance suspended for abuse.
...
Two defaults already work in your favour, and you should leave both alone:

- `HOST=127.0.0.1` in the unit file keeps the API server off the public
  interface entirely, so the reverse proxy is the only way in even if a
  firewall rule is wrong.
- `PROXY_ALLOW_PRIVATE_NETWORK` stays unset. In production the guard refuses
  private ranges, which is what stops someone using the proxy to reach into
  your VCN or read the OCI instance metadata service at `169.254.169.254`.
...
```bash
curl -u you:yourpassword https://workbench.example.com/api/healthz
curl -so /dev/null -w '%{http_code}\n' https://workbench.example.com/api/healthz   # expect 401
```

The second command is the one that matters: a 401 without credentials means the
forwarder is not open to the world. If it returns 200, stop and fix the gate
